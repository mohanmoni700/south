...
class InitiateFlow extends Action implements ResultInterface
{
    ...
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/slope.log');
        $logger = new \Zend_Log();
        ...
        if (!isset($slopeOrder) && isset($slopeOrder['id']) && $slopeOrder['id'] != '') {
            ...
            //create slope order if order does not exists
            $statusCode =  $slopeOrder['statusCode'] ?: null;
    ...
    public function createNewSlopeOrder($order)
    {
        ...
        if ($environment == Environment::ENVIRONMENT_SANDBOX) {
            ...
            $apiEndpointUrl = $this->config->getSandboxApiEndpointUrl();
        } else {
            $publicKey = $this->config->getProductionPublicKey();
            $privateKey = $this->config->getProductionPrivateKey();
            $apiEndpointUrl = $this->config->getProductionApiEndpointUrl();
        }
    ...
    public function getSlopeOrderIntent($slopeOrderId)
    {
        ...
        //https://api.sandbox.slope.so/v3/orders/{id}/intent
