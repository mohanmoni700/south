...
namespace Avalara\Excise\Framework\Interaction;

use Avalara\Excise\Framework\Interaction\MetaData\MetaDataObject;
...
class Address
{
    ...
    public function copyQuoteAddressToCustomerAddress(
        ...
        $customerAddress->setCustomAttributes($quoteAddress->getCustomAttributes());
    ...
    public function convertAvaTaxValidAddressToCustomerAddress(
        ...
        $address->addData(isset($addressData[0]) ? $addressData[0] : []);
        if ($address->hasData('line1')) {
            $street[] = $address->getData('line1');
        }
        if ($address->hasData('line2')) {
            $street[] = $address->getData('line2');
        }
        if ($address->hasData('line3')) {
            $street[] = $address->getData('line3');
        }

        $region = $this->getRegionByCodeAndCountry($address->getData('region'), $address->getData('country'));
        if ($region === null) {
            return null;
        }
        ...
        $data = array_merge($customerAddressData, [
            ...
            CustomerAddressInterface::COUNTRY_ID => $address->getData('country'),
            CustomerAddressInterface::STREET => $street,
            CustomerAddressInterface::POSTCODE => $address->getData('postalCode'),
            CustomerAddressInterface::CITY => $address->getData('city'),
            "county" => $address->getData('county'),
        ]);
    ...
    public function convertAvaTaxValidAddressToQuoteAddress(
        ...
        $originalData = $originalAddress->getData();
        $data = array_merge(
            $originalData,
            [
                QuoteAddressInterface::KEY_COUNTRY_ID => $address->getData('country'),
                QuoteAddressInterface::KEY_REGION_CODE => $address->getData('region'),
                QuoteAddressInterface::KEY_STREET => $street,
                QuoteAddressInterface::KEY_POSTCODE => $address->getData('postalCode'),
                QuoteAddressInterface::KEY_CITY => $address->getData('city'),
                "county" => $address->getData('county'),
            ]
        );
        $region = $this->getRegionByCodeAndCountry($address->getData('region'), $address->getData('country'));
        ...
        $originalAttr2 = $originalAddress->getExtensionAttributes();
        if (!empty($originalAttr2)) {
            $originalAttr = clone $originalAttr2;
            if (!empty($originalAttr) && !empty($address->getData('county'))) {
                $originalAttr->setCounty($address->getData('county'));
                $data[CustomerAddressInterface::EXTENSION_ATTRIBUTES_KEY] = $originalAttr;
            }
        }
