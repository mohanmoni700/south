...
class ProcessTaxQuote
{
    ...
    public function getRecalculatedTax($queueObject, $entityType)
    {
        ...
        if ($queueObject->getIncrementId() && $queueObject->hasData('excise_tax_response_order')) {
            ...
            $exciseTaxResponseOrderData = json_decode($exciseTaxResponseOrder, true);
        ...
        if ($entityType == "invoice") {
            ...
            $cartItems['InvoiceNumber'] = "INV" . $queueObject->getId();
            $cartItems['UserTranId'] = "INV" . $queueObject->getId();
            ...
            /* This can be used for further release based on API changes.*/
            $cartItems['InvoiceNumber'] = "CM" . $queueObject->getId();
            $cartItems['UserTranId'] = "CM" . $queueObject->getId();
            $obj->setEventBlock("CreditMemoPostCalculateTax");
            $obj->setDocType(self::SALE_REFUND_TYPE);
            $obj->setDocCode($cartItems['InvoiceNumber']);
        }
        $cartItems['TitleTransferCode'] = 'DEST';
        $cartItems['TransactionType'] = 'RETAIL';
    ...
    private function _getQuoteObjectLineItems($queueObject, $lineParam, $entityType, $customerData)
    {
        ...
        if (count($items) > 0) {
            ...
            foreach ($items as $item) {
                ...
                $itemUnitQuantityUnitOfMeasure = $product->getAttributeText('excise_unit_qty_measure');
                $productCode = $product->getAttributeText('excise_product_tax_code');
                if ($productCode == "Not Applicable") {
                    $productCode = $itemSku;
                }
                $itemUnitVolume = $product->getExciseUnitVolume();
                $itemUnitVolumeUnitOfMeasure = $product->getAttributeText('excise_unit_vol_measure');
                $lineitems = [
                    ...
                    'UnitQuantityUnitOfMeasure' => $itemUnitQuantityUnitOfMeasure,
                    'UserData' => '',
                    ...
                    'UnitVolumeUnitOfMeasure' => $itemUnitVolumeUnitOfMeasure,
                    'SaleType' => $saleType
                ];
    ...
    private function _getLineItems(
        ...
        if (count($items) > 0) {
            ...
            foreach ($items as $item) {
                ...
                $itemUnitOfMeasure = $item->getProduct()->getAttributeText('excise_unit_of_measure');
                $itemUnitQuantity = $item->getProduct()->getExciseUnitQuantity();
                if ($item->getProductType() == BundleProductType::TYPE_CODE) {
                    $itemSku = $product->getData('sku');
                }
                $itemUnitQuantityUnitOfMeasure = $item->getProduct()->getAttributeText('excise_unit_qty_measure');
                $productCode = $item->getProduct()->getAttributeText('excise_product_tax_code');
                if ($productCode == "Not Applicable") {
                    $productCode = $itemSku;
                }
                $itemUnitVolume = $item->getProduct()->getExciseUnitVolume();
                $itemUnitVolumeUnitOfMeasure = $item->getProduct()->getAttributeText('excise_unit_vol_measure');
                ...
                $lineitems = [
                    ...
                    'UnitPrice' => $itemUnitPrice,
                    'BilledUnits' => $itemQty,
                    //'LineAmount' => $itemUnitPrice  * $itemQty,
